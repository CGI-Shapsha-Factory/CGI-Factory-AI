...
description: Prend en charge une anomalie de recette : requalifie en évolution si le code respecte la spécification, sinon analyse l'impact, enquête dans le code, corrige et referme avec la trace à jour.
---
...
Linear à jour - cf. `references/regles-recette.md`).
...
- **L'anomalie existe** : identifiant Linear fourni par le développeur (ou retrouvée par
  ...
  reprendre via `/recette:creation-anomalie`.
...
3. **Analyser l'impact.** À partir de la feature (son entrée du registre et ses use cases),
   ...
   `references/regles-recette.md`), **alerter** qu'elle doit remonter au niveau central plutôt
...
Étape suivante : `/recette:creation-evolution` si l'anomalie a été requalifiée (geste du PO), sinon reprendre la recette de la feature.
