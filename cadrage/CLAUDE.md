...
- **Le texte rendu par un hook n'est pas interprété comme du markdown** (vérifié à l'usage : tableaux pipe et `**` s'affichaient littéralement). Le script **convertit donc en colonnes alignées** avant de rendre : un skill par bloc, nom en colonne de 30, rôle habillé à 96 caractères, porte entre crochets en dessous. Le `SKILL.md` reste écrit en markdown - c'est la conversion qui s'adapte, pas la source. Si on ajoute une colonne à un tableau, seuls `skill` et `porte...` sont reconnus par en-tête ; une colonne `#` est ignorée (elle ne fait que numéroter des lignes déjà ordonnées).
