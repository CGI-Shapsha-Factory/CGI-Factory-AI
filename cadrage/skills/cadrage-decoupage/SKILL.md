...
- Base amont : `cadrage-out/product-brief.md` et `cadrage-out/glossaire.md` (couverture du
  périmètre IN, use case sans objectif, dépendance citée introuvable, terme hors glossaire).
- La passe précède la revue de couplage : elle durcit la proposition avant l'arbitrage humain.
  Chaque use case cité se nomme par son intitulé complet en langage naturel suivi de sa
  référence entre parenthèses, jamais un `UCn` nu.
- Une correction qui change matériellement le découpage applique la règle de reset d'arbitrage
  ci-dessous.
