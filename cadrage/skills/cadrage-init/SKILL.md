...
**`cadrage-out/`** à la racine pour les documents générés **et le manifeste committé**
(`cadrage-out/manifest.json`, qui **voyage avec le repo**), un dossier `cadrage-out/prompts/` pour les
...
3. **Écrire le manifeste** `cadrage-out/manifest.json` (squelette ci-dessous ;
...
- `.factory/` (avec `cadrage/`), `cadrage-out/` (avec le **manifeste committé** `manifest.json`,
  `source-contexte/`, `features-fonctionnels-brief/` et `maquette-de-claude-design/`) et
  `cadrage-out/prompts/` existent.
- `.factory/cadrage/` contient les 6 gabarits installés (git-ignoré).
- Le **manifeste vit sous `cadrage-out/manifest.json`** (committé, voyage avec le repo) — plus jamais
  sous `.factory/`.
...
- `cadrage-out/manifest.json` reparse sans erreur, `phase = "init"`.
