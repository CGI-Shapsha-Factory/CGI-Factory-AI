...
(parcours) et `architecte-out/design-impact.md` (architecte) ; la **source du design system validé** — **soit**
une **référence Claude Design** (URL/identifiant), **soit** un **export committé** dans le repo (zip ou dossier
avec les tokens, ex. `tokens.css`) — à fournir par l'humain.
...
2. **Porte humaine : validation du système généré** (porte 2, jamais automatisée). Le designer **valide**
   le design system (cohérent avec la couverture, la direction stylistique et la stack), qu'il ait été produit
   dans **Claude Design** ou fourni comme **export committé**. Capter la **source** dans
   `design.design_system_ref` et **noter sa nature** dans `design.design_system_kind`
   (`claude_design_ref` **ou** `committed_export`). Si c'est un **export committé**, **vérifier qu'il existe
   à la racine du repo** ; s'il est absent, le dire en clair (**fichier manquant**) et ne pas sceller. Le
   skill ne passe **jamais** `design.design_validated` à vrai de lui-même ; il le propose, l'humain confirme.
3. **Produire le handoff design** → `designer-out/design-guidelines.md` (gabarit
   `.factory/templates/design-guidelines.md`) : **source du design system validé** (réf. Claude Design **ou**
   export committé) **et son mode de synchronisation** — `/design-sync` **depuis l'export committé s'il est
   présent**, sinon depuis la réf. Claude Design —, **règles d'états** (par écran), **patterns d'erreur**
   (validation à la sortie du champ, format API → messages par champ), **socle d'accessibilité** (niveau
   visé), et la règle **aucune valeur de style en dur**. MAJ `design.guidelines_path`.
...
Le contrat de design prêt à transmettre = la **source du design system validé** — **réf. Claude Design**
(pont `/design-sync`) **ou export committé** dans le repo (auto-portable) — + les **guidelines** (règles
d'états, patterns d'erreur, socle a11y). **Si seule une réf. externe est fournie**, prévenir en clair que la
fabrication exigera l'accès au compte Claude Design ; **si un export est committé**, la phase design est
auto-portable. C'est ce que l'Assembleur grave dans la constitution / le `claude.md` / la CI (voir §6 de la
spec : forcer `/design-sync` **depuis la source présente**, interdire les valeurs de style en dur, contrôler
les états et patterns d'erreur).
...
- **Le design system vit dans Claude Design *ou* dans un export committé au repo** ; le plugin ne le
  régénère pas. Un export committé rend la phase auto-portable ; une réf. externe exige l'accès au compte.
...
`designer-out/` — **plus l'export du design system s'il est committé au repo**. La phase suivante lit le
**repo committé**, pas ta session.
