...
## Sorties (3)
1. **Prompt Claude Design** (`designer-out/prompts/prompt-<N>.md`, **fichier plat**, **corps seul prêt à coller - plein texte, aucun Markdown**) - fait naître le design system. **Direction visuelle délibérée et CONCRÈTE (anti-slop)** : valeurs **nommées** écrites dans le prompt - palette en **hex + rôle**, **polices nommées**, espacement/rayons - déduites du domaine (ou marque client), **jamais** le violet/indigo par défaut ni polices par défaut, **jamais** les clichés d'IA (bloc "À éviter absolument" + phrase de verrou "tout choix non spécifié = un défaut générique"). *"Silence = Claude defaults"* : on ne laisse aucun choix esthétique au défaut.
