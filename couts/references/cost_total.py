...
def main(argv):
    ...
    # Garde-fou path traversal : la racine resolue doit etre un dossier EXISTANT. Un argument
    # fantaisiste (ex. "../../tmp") ne doit pas faire creer une arborescence .factory/ hors projet.
    if not os.path.isdir(root):
        print(f"cost_total: racine introuvable ou invalide: {root}", file=sys.stderr)
        return 2
    # Creer .factory/couts/ si absent (peut tourner avant couts-init)
    outdir = os.path.join(root, ".factory", "couts")
    os.makedirs(outdir, exist_ok=True)
    # Lecture defensive : journal/table de prix manquant ou corrompu -> message clair, pas de trace.
    try:
        records = cost_report.load_journal(root)   # deja deduplique par key
        pdate = cost_report.price_table_date(root)
    except (OSError, ValueError) as exc:  # ValueError couvre json.JSONDecodeError
        print(f"cost_total: lecture du journal/table de prix impossible: {exc}", file=sys.stderr)
        return 2
