...
Produit les **deux restitutions** de coût à partir du journal (`.factory/costs/`) et de la config
(`.factory/cost/cost-config.json`). **Ne jamais présenter la simulation comme du réel.**

## Pré-requis (vérification silencieuse)
`couts-init` a tourné (`.factory/cost/` existe). Sinon, orienter en clair vers `/couts:couts-init`.
...
1. Lancer `python "${CLAUDE_PLUGIN_ROOT}/references/cost_report.py" <racine-projet>` (adapter `py -3` si
   ...
   valorise via la table de prix datée, lit la config pour le coût réel, et écrit
   `.factory/cost/rapport-couts.md`.
...
Dire en clair qu'il faut compléter `.factory/cost/cost-config.json` : nombre de développeurs par forfait
