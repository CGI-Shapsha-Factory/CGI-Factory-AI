...
**hooks optionnels** (ex. `after_tasks` -> `creation-taches-par-phase-de-spec`, `after_implement` ->
...
lancer `creation-taches-par-phase-de-spec` pour synchroniser les sous-tickets `Task` (le hook ne parle jamais à Linear).
