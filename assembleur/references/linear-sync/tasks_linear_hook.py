...
(`## Phase N:`), POUSSE l'agent (message `decision:block`) a lancer `/assembleur:creation-taches-par-phase-de-spec`,
...
def cmd_posttooluse():
    ...
    if not isinstance(linear, dict) or not linear.get("team"):
        return _block(
            ...
            f"/assembleur:creation-taches-par-phase-de-spec pour creer les sous-tickets Task (un par phase, label Task, "
    ...
    return _block(
        ...
        f"Linear ({phases_txt}). Lance /assembleur:creation-taches-par-phase-de-spec - il verifie sur Linear (par "
