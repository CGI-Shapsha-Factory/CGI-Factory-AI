...
- `creation-tasks-linear` - **sous-tickets par phase (2ᵉ niveau de Task)** : à lancer **après
...
- `install-speckit` - **pont vers SpecKit** : pose SpecKit dans le repo cible via
  ...
  (`after_tasks`->`creation-tasks-linear`, `after_implement`->`update-issue-linear`) -, **pose le hook
  ...
  `creation-tasks-linear` s'il manque un sous-ticket `Task` pour une phase - **le hook ne parle jamais à
