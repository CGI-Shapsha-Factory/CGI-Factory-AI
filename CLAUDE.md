...
| `couts/` | **Transversal** (pas une phase) — mesure des coûts **en temps réel, par tour** : coût réel vs simulation, ventilé par phase / feature / tier + ligne Cowork ; hook `Stop` (par tour, 4 catégories, dédup curseur) + `SessionEnd` (réconciliation) | **construit** | voir `couts/CLAUDE.md` |
...
| **couts** (transversal) | transcript de chaque tour Claude Code (hook `Stop`) + `manifest.feature_sequence` + `.factory/cost/{price-table,cost-config}.json` | `.factory/costs/<aaaa-mm>/<session-id>.jsonl` (**une ligne/tour**, temps réel) ; `.factory/cost/rapport-couts.md` ; hooks `Stop`+`SessionEnd` dans `.claude/` (racine) | `costs` |
