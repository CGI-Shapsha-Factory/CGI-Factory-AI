...
| `assembleur/` | Convergence des 3 contrats par feature + **tickets Linear (1/feature, via MCP linear-prism)** + amorçage repo SpecKit (constitution, CLAUDE.md, briefs 3-faces, glossaire consolidé, MEMORY.md, seeds spec.md, CI backstop) ; **n'écrit pas dans le repo cible** (2 exceptions bornées : Linear externe, `specify init`) | **construit** | voir `assembleur/CLAUDE.md` |
| `couts/` | **Transversal** (pas une phase) — mesure des coûts **à la fin de session** (sans latence par tour) : coût réel vs simulation, ventilé par phase / feature / tier + ligne Cowork ; hook `SessionEnd` (un enregistrement par message, 5 catégories, dédup globale par clé) | **construit** | voir `couts/CLAUDE.md` |
...
| **assembleur** | `cadrage-out/` + `architecte-out/` (tech-stack, components, standards, decisions/, drivers-quality) + `designer-out/` (design-guidelines, coverage-report) | `assembleur-out/` (paquet de handoff) : `pre-constitution.md`, `features/<id>-*.spec-seed.md`, `feature-map.md`, `technical-context.md`, `CLAUDE.md`, `ci/tests.yml`, `memory/{MEMORY,domain,architecture,design,features}.md`, `coherence-report.md`, `attack-plan.md` ; **+ tickets Linear** (1/feature, MCP linear-prism ; `linear-drafts.md` en repli) ; **+ `specify init`** dans le repo (install-speckit) | `assembly`, `linear`, `speckit` |
| **couts** (transversal) | transcript complet à la fin de session (hook `SessionEnd`) + `manifest.feature_sequence` + `.factory/couts/{price-table,cost-config}.json` | `.factory/couts/<aaaa-mm>/<session-id>.jsonl` (**un enregistrement/message**, réécrit à chaque fin de session) ; `.factory/couts/rapport-couts.md` ; `.factory/couts/bilan-couts.md` (bilan partageable) ; hook `SessionEnd` dans `.claude/` (racine). **Tout `.factory/couts/` git-ignoré** (individuel, jamais poussé). | `costs` |
